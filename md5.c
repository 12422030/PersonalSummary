...
{
	UINT i, j;
	for(i=0, j=0; i<nLength; i+=4, j++)
		Output[j] = (ULONG)Input[i+0] << 0*8 |
					(ULONG)Input[i+1] << 1*8 |
					(ULONG)Input[i+2] << 2*8 |
					(ULONG)Input[i+3] << 3*8;
}
...
void DWordToByte(BYTE* Output, DWORD* Input, UINT nLength )
{
	...
	{
		Output[j+0] = (UCHAR)((Input[i] >> 0*8) & 0xff);
		Output[j+1] = (UCHAR)((Input[i] >> 1*8) & 0xff);
		Output[j+2] = (UCHAR)((Input[i] >> 2*8) & 0xff);
		Output[j+3] = (UCHAR)((Input[i] >> 3*8) & 0xff);
	}
}
...
void Final(char*buf)
{
	...
	for ( i=0; i < nMD5Size; i++) 
	{
		...
		sprintf(temp, "%02X", lpszMD5[i]);
		strcat(abuf, temp);
	}
	memcpy(buf, abuf, 32) ;
}

void LiweiMD5(BYTE * Output, BYTE * Input, UINT nInputLen)
{
	BYTE buff[4096];
	memcpy(buff,Input,nInputLen);
	Init();
	Update(buff, nInputLen);
	Final((char*)buff);
	memcpy((char*)Output,(char*)buff, 32);
	Output[32] = 0 ;
}
